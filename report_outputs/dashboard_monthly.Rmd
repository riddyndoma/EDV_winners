...
### Complement
```{r regularity, fig.height=9,fig.width=15,fig.align='center'}
...
# df_jul20 <- df_jul20[sample(nrow(df_jul20), 1000),] 
# df_aou20 <- df_aou20[sample(nrow(df_aou20), 1000),]
...
# fig <- ggplot2::ggplot(data = people_regularity)+geom_line(aes(x=month_val, y=i, group=i),size=3)
# fig <- fig + theme(legend.position="bottom", legend.direction="horizontal", legend.title = element_blank())
# fig <- fig + theme_classic()+labs(x="",y="ID Guéris", title = "La régularité des guéris au suivi", subtitle = )
# fig <- fig + scale_colour_manual(values=colour)
# 
# fig <- fig + theme(legend.background = element_rect(fill="#f2f4c0",
#                                   size=1, linetype="solid", 
...
# people_regularity <- people_regularity %>%
  # mutate(month_val = paste0("m-",people_regularity$month_val))
  rand_people <- people_regularity[sample(nrow(people_regularity), 50),] %>%
    mutate(presence=v,
           mois=month_val,
           id=i)
  # group_by(month_val)
ggplot(data = rand_people, aes(x=mois, y=id, fill=presence)) + 
  geom_tile()
