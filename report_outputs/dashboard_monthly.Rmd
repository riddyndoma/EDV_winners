...
#    select(uuid,date_of_reporting,gender) %>%
...
# geom_bar(stat="identity")+theme_classic() +labs(x="",y="Effectifs", title = "", subtitle = )
# g <- g+geom_text( aes(label = n, y = n),position = position_dodge(0.9),vjust = 0)+facet_grid(~gender)
# g <- g+scale_fill_poke(pokemon = 'Quilava', spread = 6)
# g <- g+theme(axis.text.x = element_blank(),axis.ticks.x = element_blank())
# g <- g+theme(legend.background = element_rect(fill="#b2c8d6",
#                                   size=0.5, linetype="solid",
...
### Complement
```{r}
test4 <- getRegularityFollowups(current_clean_contacts,current_clean_followups,4)
test3 <- getRegularityFollowups(current_clean_contacts,current_clean_followups,3)
test3 <- test3[sample(nrow(test3), 1000),] %>%
  # test3 %>%
  # head(40) %>%
  filter(v==1)
test4 <- test4[sample(nrow(test4), 1000),] %>%
  # head(40) %>%
  filter(v==1)
people_regularity=rbind(test4,test3) %>%
  # head(20) %>%
  arrange(month_val) %>%
...
people_regularity <- people_regularity %>%
  ...
  rand_people <- people_regularity[sample(nrow(people_regularity), 40),] %>%
    mutate(presence=v,
           mois=month_val,
           id=i)
  # group_by(month_val)
...
```{r survivors-followed-weekly, fig.width=9,fig.align='center',fig.height=4}
...
men_followed_monthly=rbind(men_followed_avril2021,men_followed_mars2021,men_followed_fev2021,
...
women_followed_monthly=rbind(women_followed_avril2021,women_followed_mars2021,women_followed_fev2021,
...
all_followed_monthly=rbind(all_followed_avril2021,all_followed_mars2021,all_followed_fev2021,
...
fig <- fig + theme(legend.position="bottom", legend.direction="horizontal", legend.title = element_blank())
fig <- fig + theme_classic()+labs(x="",y="Nombre de gueris", title = "Guéris suivis par mois, selon le sexe", subtitle = )
fig <- fig + scale_colour_manual(values=colour)
