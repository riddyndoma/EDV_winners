...
#     see below commented out code in Follow Ups section for an example
    ...
    date_30d_ago <- format((Sys.Date() - 33), "%Y-%m-%dT23:59:59.999Z")
...
contacts <- as_tibble(fromJSON(json_contacts, flatten = TRUE)) %>%
  filter(active == TRUE) %>% #Les contacts actifs
  filter(as.Date.character(followUp.endDate) >= Sys.Date()) %>% #Contacts dont la date de fin de suivi depasse la date du jour(systeme)
